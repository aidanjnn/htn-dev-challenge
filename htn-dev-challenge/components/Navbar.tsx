...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-4 left-0 right-0 z-50 flex justify-center px-4">
      <div
        className={[
          "flex items-center rounded-full border border-white/20 bg-white/10 py-3 backdrop-blur-xl shadow-lg shadow-black/20",
          "transition-all duration-500 ease-in-out",
          expanded ? "justify-between gap-4 px-8" : "gap-6 px-6 sm:gap-8 sm:px-8",
        ].join(" ")}
        style={{
      ...
      >
        ...
        <div
          className={[
            "hidden items-center justify-center gap-6 overflow-hidden whitespace-nowrap transition-all duration-500 sm:flex",
            expanded ? "flex-1 opacity-100" : "max-w-0 opacity-0",
          ].join(" ")}
        >
          {NAV_LINKS.map((link) => (
            <a
        ...
        <div className="hidden items-center gap-4 sm:flex" aria-label="Social links">
          {/* LinkedIn */}
          <a href="https://www.linkedin.com/company/hack-the-north/about/" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="text-white/70 transition-colors hover:text-white">
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
            </svg>
          </a>
          {/* Instagram */}
          <a href="https://www.instagram.com/hackthenorth/" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-white/70 transition-colors hover:text-white">
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          {/* X / Twitter */}
          <a href="https://x.com/HackTheNorth" target="_blank" rel="noopener noreferrer" aria-label="X" className="text-white/70 transition-colors hover:text-white">
            <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
          </a>
          {/* TikTok */}
          <a href="https://www.tiktok.com/@hackthenorth" target="_blank" rel="noopener noreferrer" aria-label="TikTok" className="text-white/70 transition-colors hover:text-white">
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
